...
export class Utils {
    ...
    public static checkIsOwner(stubHelper: StubHelper, idOperator: string): boolean {
        let cid = stubHelper.getClientIdentity();
        return cid.assertAttributeValue('operator', idOperator);
    }

    public static async invokedByChaincode(stubHelper: StubHelper, chaincodeName: string, functionName: string = undefined): Promise<boolean> {
        await Helpers.checkArgs<{ chaincodeName: string, functionName: string }>(
            ...
            Yup.object().shape({
                ...
                functionName: Yup.string().default(null).required(),
            }));

        const filename = path.join(process.cwd(), 'node_modules/fabric-shim/lib/protos/peer/chaincode.proto');
        const _chaincodeProto = this.load(filename).protos;

        // @ts-ignore
        const {proposal} = stubHelper.getStub().getSignedProposal();
        // @ts-ignore
        const input = _chaincodeProto.ChaincodeInput.decode(_.clone(proposal.payload.input));

        const args = input.args.map((entry: any) => entry.toBuffer().toString('utf8'));

        const idxOfCC = args[0].indexOf(chaincodeName);
        const idxOfFunc = args[0].indexOf(functionName);

        if (_.isUndefined(functionName)) {
            const isInvokedFromUndefined: boolean = idxOfCC > -1;
            return Promise.resolve(isInvokedFromUndefined);
        }

        const isInvoked: boolean = idxOfCC < idxOfFunc && idxOfCC > -1;
        return Promise.resolve(isInvoked);
    }
